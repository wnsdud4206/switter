...
// 수정2, setDoc + { merge: true }!!!
// users collection XXX -> notification collection OOO, notification XXX
notification collection
unConfirmed: {
  <sweetId>: {
    sweetComments: [<commentId>, ...],
    sweetLikes: [<userId>, ...],
    commentLikes: {
      <commentId>: {
        id: [<userId>, ...], 
        commentLikesLastUpdate: Date.now()
      },
      ...
    },
    sweetCommentsLastUpdate: Date.now(),
    sweetLikesLastUpdate: Date.now(),
  },
  ...
},
confirmed: "
...
unConfirmed: {
  sweetComments: {
    <sweetId>: {
      id: [commentId, ...],     // arrayUnion()&arrayRemove()
      lastUpdate: Date.now()
    },
  },
  sweetLikes: {
...
const notification = async (
  ...
  const d = doc(dbService(), "notifications", creatorId);
  // test
